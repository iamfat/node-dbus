	...
	for (var methodName in self.methods) {
		...
		for (var index in method['in']) {
			...
			if (typeof type === 'object')
				type = 'a' + (type.__sig || '{sv}');
			if (arg.name)
				introspection.push('<arg direction="in" type="' + type + '" name="' + arg.name + '"/>');
			else
				introspection.push('<arg direction="in" type="' + type + '"/>');
		}
		...
		if (method['out']) {
			...
			var type = out_def.type;
			if (typeof type === 'object')
				type = 'a' + (type.__sig || '{sv}');
			if (out_def.name)
				introspection.push('<arg direction="out" type="' + type + '" name="' + out_def.name + '"/>');
			else
				introspection.push('<arg direction="out" type="' + type + '"/>');
		}
