...
var ServiceObject = module.exports = function(service, objectPath) {
	...
	self.propertyInterface.addMethod('GetAll', {
		...
		out: function (_, __, args) {
			var def = Utils.Define(Object, 'properties');
			if (!args || args.length != 1) return def;
			var interfaceName = args[0];
			var iface = self['interfaces'][interfaceName];
			if (iface) {
				def.type = {};
				Object.keys(iface.properties).forEach(function (propName) {
					var prop = iface.properties[propName];
					if (prop && prop.type && prop.type.type) {
						def.type[propName] = prop.type.type;
					} else {
						def.type[propName] = 'v';	// default is variant
					}
				});
			}
			return def;
